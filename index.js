...
const { token } = require('./config.json'); //importing config.json
...
client.on('messageCreate', (message) =>{
    if(message.author.id === client.user.id) return; //makes sure bots dont talk to each other

    if(message.content === "ping"){ //if the user types in 'ping' bot will respond with 'pong'
        message.reply("pong");
    }
...
client.on('interactionCreate', async interaction => {
    ...
    if(isGameOver()){
        ...
        if(!user){ //if user doesn't exist in database
            user = await TicTacToe.create({user_id: interaction.user.id});
        }

        user.increment('score'); //increments user's score
    ...
    interaction.update({ //updates the state of the board
        components : makeGrid() 
    })
...
client.login(token); //invoking the config.json file that stores token